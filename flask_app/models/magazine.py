...
class Magazine:
    ...
    def get_all_magazines(cls):
        query = "SELECT magazines.*, users.firstName ,users.lastName FROM magazines LEFT JOIN users ON magazines.user_id = users.id;"
    ...
    def get_magazine_by_id(cls, data):
        query = "SELECT magazines.*, users.firstName ,users.lastName FROM magazines LEFT JOIN users ON magazines.user_id = users.id WHERE magazines.id=%(id)s;"
        results = connectToMySQL(cls.db_name).query_db(query, data)
        if results:
            return results[0]
        return False
    ...
    @classmethod
    def check_subscription(cls, data):
    ...
    def all_subscribed(cls, data):
        #get all subscribed of magazins magazines with first and last name of user
        query = "SELECT subcribes.*, magazines.title, users.firstName, users.lastName FROM subcribes LEFT JOIN magazines ON subcribes.magazine_id = magazines.id LEFT JOIN users ON subcribes.user_id = users.id WHERE magazines.id=%(id)s;"
        results = connectToMySQL(cls.db_name).query_db(query, data)
        subscribed = []
        if results:
            for sub in results:
    ...
    def get_all_subscriptions(cls, data):
        query = "SELECT subcribes.*, magazines.title, users.firstName, users.lastName FROM subcribes LEFT JOIN magazines ON subcribes.magazine_id = magazines.id LEFT JOIN users ON subcribes.user_id = users.id WHERE subcribes.user_id=%(id)s;"
        results = connectToMySQL(cls.db_name).query_db(query, data)
        print(results)
        subscribed = []
        if results:
    ...
    @classmethod
    def calculate_total_subscriptions(cls, subscriptions, number):
        total = {}
        count = 0
        #id is the magazine id
        for sub in subscriptions:
            count = 0
            for num in number:
                if sub['magazine_id'] == num['magazine_id']:
                    count += 1
            total[sub['magazine_id']] = count
        return total
